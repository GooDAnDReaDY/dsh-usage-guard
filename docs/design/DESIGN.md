...
- **2026-09-18 (v0.1.10, #13, #14):**
  - **Выравнивание идентичности пакета (#13):** `export const name` в `lib/index.js` приведен к каноническому полному имени `@goodandready/dsh-usage-guard`, совпадающему с `package.json`, `cordis.patch.yml` и клиентским загрузчиком. Настройки сохраняют пространство `NS = 'dsh-usage-guard'`. Добавлен тест 4-стороннего совпадения идентичности.
  - **Корректный контракт сервиса локали (#14):** Резолвер языка `getActiveLocale` в `lib/client.js` переведен на чтение актуального контракта `LocaleSnapshot` сервиса `@deepseek-ai/dsh-client-locale` через `getLocale()` / `getSnapshot()` и поле `active` вместо несуществующих методов.
- **2026-09-14 (#9):**
