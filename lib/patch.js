...
export function patchRegistry(registry, guard) {
  ...
  // Save the current map.set (may be the prototype method or a previous patched version).
  // We restore via explicit assignment — relying on `delete` to restore the prototype method
  // is fragile when the environment mutates Map.prototype.
