...
export function findDshCliEntry(argv1 = process.argv[1]) {
  const value = argv1;
  if (value === undefined || value === '') return undefined;
  let entry = value.startsWith('file:') ? fileURLToPath(value) : resolve(process.cwd(), value);
  if (!existsSync(entry)) return undefined;
  try {
    entry = realpathSync(entry);
  } catch {
    // Ignore symlink resolution errors and continue with unresolved path
  }
