...
export function apply(ctx, config) {
  ...
  // О каждом сочетании хода, шага и набора полей жалуемся один раз: выжимка
  // считается проигрыванием журнала заново, и без этого одна и та же порция
  // въезжала бы в журнал при каждом открытии истории.
  const told = new Set()
  ...
  const guard = (event) => {
    ...
    if (live.report !== false) {
      const seen = `${String(found.turn)}/${String(found.step)}/${bad.join(',')}`
      if (!told.has(seen)) {
        told.add(seen)
