...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function getActiveLocale(c) {
      try {
        const cur = (c?.locale && typeof c.locale.get === 'function' ? c.locale.get() : c?.locale?.current) || '';
...
.ug-btn-accent{background:var(--dsw-alias-state-brand-primary);color:#ffffff;border-color:transparent}
...
`;
    ...
    function UpdaterSection({ t }) {
      ...
      const currentVer = status?.currentVersion || '0.1.8';
      ...
      return React.createElement(
        ...
        updateAvailable
          ? React.createElement(
              'div',
              { style: { marginTop: '8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', background: 'rgba(245,158,11,0.08)', borderRadius: '6px', border: '1px solid rgba(245,158,11,0.2)' } },
    ...
    function refreshMirrorUntilVisible(c) {
      const visible = () => {
        try {
          const s = (c?.get && c.get('lanSettings')) || c?.settingsScope;
