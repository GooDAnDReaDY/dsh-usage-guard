    ...
    function PluginCard({ ctx }) {
      ...
      return React.createElement('div', { className: 'ug-card' },
        ...
        React.createElement('button', {
          ...
          React.createElement(Chevron, { className: 'ug-chev' + (expanded ? ' ug-chev-open' : '') })
        ),
            ...
            React.createElement('div', { className: 'ug-field' },
              React.createElement('label', { className: 'ug-row' },
                ...
                React.createElement('input', {
                  ...
                  onChange: (e) => setDraft({ ...draft, report: e.target.checked }),
                })
              ),
    ...
    module.exports.apply = function apply(ctx) {
      try {
        ctx.locale.register(NS, {
          ...
          ru: {
            title: 'Защита истории токенов',
            subtitle: 'Санитайзер расхода токенов и защита от сбоев истории',
            active: 'АКТИВЕН',
            reportOnly: 'ТОЛЬКО ЛОГ',
            saved: 'Сохранено',
            loading: 'Загрузка…',
            unavailable: 'Пространство настроек недоступно',
            save: 'Сохранить',
            saving: 'Сохранение…',
            repair: 'Автоматическое исправление расхода',
            repairDesc: 'Подставлять ноль вместо повреждённых счётчиков до сложения для предотвращения блокировки истории диалогов.',
            report: 'Диагностические предупреждения',
            reportDesc: 'Логировать номер хода, шага и исходные данные при обнаружении битой порции расхода.',
          },
          zh: {
            title: '用量与会话防崩守卫',
            subtitle: 'Token 用量数据清洗与历史记录防白屏保护',
            active: '已激活',
            reportOnly: '仅记录',
            saved: '已保存',
            loading: '加载中…',
            unavailable: '设置作用域不可用',
            save: '保存',
            saving: '保存中…',
            repair: '自动修复用量计数',
            repairDesc: '在累加前将缺失或非数字计数置为 0，彻底防止 NaN 导致会话历史永久白屏。',
            report: '记录诊断告警',
            reportDesc: '当检测到异常用量数据包时，在控制台记录轮次、步骤和原始数据。',
          },
        });
      } catch (_) {}
      ...
      if (typeof ctx.effect === 'function') {
        ctx.effect(() => () => clearTimeout(timer), 'dsh-usage-guard: fallback section');
      }
    };
