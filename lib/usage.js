 ...
 */
const clampedMap = new WeakMap()

export function clampedDetails(usageOrBad, options = {}) {
  if (Array.isArray(usageOrBad)) {
    return clampedMap.get(usageOrBad) ?? null
  }
  const usage = usageOrBad
  if (!usage || typeof usage !== 'object') return null
  const maxStepTokens = typeof options === 'number' ? options : (Number(options?.maxStepTokens) > 0 ? Number(options.maxStepTokens) : 0)
  if (maxStepTokens <= 0) return null
  let clamped = null
  for (const field of FIELDS) {
    const value = usage[field]
    if (typeof value === 'number' && Number.isFinite(value) && value > maxStepTokens) {
      if (!clamped) clamped = {}
      clamped[field] = { from: value, to: maxStepTokens }
    }
  }
  return clamped
}
...
export function borrowed(usage, field) {
  ...
  // 1. Check the canonical field itself first if passed as a string or float (e.g. "123" or 15.4)
  const selfNum = coerceNumber(usage[field])
  if (selfNum !== undefined) return selfNum

  // 2. Search known alias names
  for (const alias of ALIASES[field] ?? []) {
    const num = coerceNumber(usage[alias])
    if (num !== undefined) return num
  }

  // 3. Search nested cache details (OpenAI-style: prompt_tokens_details.cached_tokens / cachedTokens)
  ...
  // 4. Search nested cache creation details (cache_creation_input_tokens)
...
export function complaint(found, bad, options = {}) {
  ...
  const taken = Array.isArray(bad)
    ? bad
        .map((field) => {
          const clamped = options?.clamped ?? clampedMap.get(bad) ?? clampedDetails(usage, options)
          if (clamped?.[field]) {
            return `${field} clamped abnormally large (${clamped[field].from} -> ${clamped[field].to})`
          }
          const val = usage?.[field]
          if (maxStepTokens > 0 && typeof val === 'number' && val > maxStepTokens) {
            return `${field} clamped abnormally large (${val} -> ${maxStepTokens})`
          }
