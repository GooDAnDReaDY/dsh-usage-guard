// Как устроена порция расхода в событиях сессии.
//
// Харнесс слушает шину событий сессии и собирает статистику по двум типам
// записей:
//
//   1. Потоковые порции ответа ассистента:
//      `assistant/chunk` -> `event.data.chunk.usage`
//
//   2. Финальное сообщение ассистента:
//      `assistant/message` -> `event.data.usage`
//
// В обоих случаях ядро ждёт четыре поля:
//
...
//
// Если первые два поля пропущены, не являются числом или содержат нечисловую строку,
// их сложение с нулём даёт NaN. Из-за этого одна битая порция валит не просто
